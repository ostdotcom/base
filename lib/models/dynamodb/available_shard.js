...
AvailableShard.prototype = {
  ...
  createShardTable: function (ddbObject, shardTableName, tableSchema) {
    ...
    return ddbObject.call('createTable', tableSchema);
  },
   ...
   *
   */
  getShardInfo: async function (params) {
    const oThis = this
      , ddbObject = params.ddb_object
      , shardName = params.shard_name
      , queryParams = {
    ...
    if (response.isSuccess() && response.data.Items[0]) {
      ...
      return responseHelper.successWithData({
        [resObj[availableShardConst.SHARD_NAME]['S']]: {
          ...
          [availableShardConst.ALLOCATION_TYPE]: availableShardConst.getInverseShardTypes()[Number(resObj[availableShardConst.ALLOCATION_TYPE]['S'])]
        }
      })
    } else {
      return responseHelper.errorWithData("available shard ::  getShardInfo :: failed response");
    }
  },
    ...
    try {
      for (let ind = 0; ind < ids.length; ind++) {
        ...
        if (object.shard_type !== availableShardConst.all) {
          ...
          keyConditionExpression = keyConditionExpression +  " AND #al = :v2";
