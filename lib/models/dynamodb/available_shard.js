...
AvailableShard.prototype = {
  ...
  hasShard: async function (params) {
    ...
    try {
      ...
      queryParams.RequestItems[availableShardConst.getTableName()] = {Keys: keys};

      const response = await ddbObject.call('batchGetItem', queryParams);

      if (response.isFailure()) {
        return responseHelper.errorWithData("error in available_shards :: hasShard :: BatchGetItem failed");
      }
      ...
      for (let ind = 0; ind < responseArray.length; ind++) {
        ...
        dataResponse[resp[availableShardConst.SHARD_NAME]['S']] = {has_shard: true};
      }
    } catch (err) {
