...
describe('lib/formatter/response_helper', function(){
  ...
  it('Should have all the expected keys in response hash', function(){
    ...
    let responseData = responseHelper.error('test_1', api_error_key, {}).toHash();

    assert.equal(true, responseData.hasOwnProperty('success'));
    assert.equal(true, responseData.hasOwnProperty('code'));
  ...
  it('Should have all the expected keys and values when called successWithData', function(){
    ...
    let responseData = responseHelper.successWithData(testHash).toHash();
  ...
  it('Should return an array for param in err object', function(){
    let responseData = responseHelper.paramValidationError('test_1', api_error_key, [param_error_key], {}).toHash();

    assert.equal(true, responseData.err.param instanceof Array);
  });

  it('Should return code as per api_error_config in response hash', function(){

    assert.equal(apiErrorConfig[api_error_key].http_code, responseHelper.error('test_1', api_error_key, {}).toHash().code);
  });

  it('Should return code in err as per input error code', function(){

    assert.equal(apiErrorConfig[api_error_key].code,
      responseHelper.error('test_1', api_error_key, {}).toHash().err.code);
  });

  it('Should return msg in err as per input error code', function(){

    assert.equal(apiErrorConfig[api_error_key].message,
      responseHelper.error('test_1', api_error_key, {}).toHash().err.msg);
  });

  it('Should return internal_id in err as per input error code', function(){

    assert.equal('test_1',
      responseHelper.error('test_1', api_error_key, {}).toHash().err.internal_id);
  });

  it('Should have key count of 3 in response hash', function(){

    assert.equal(3,
      Object.keys(responseHelper.error('test_1', api_error_key, {}).toHash()).length);
  });

  it('Should have key count of 4 in err object of response hash', function(){

    assert.equal(4,
      Object.keys(responseHelper.error('test_1', api_error_key, {}).toHash().err).length);
  });

  it('Should return name in param object as per code in input', function(){

    assert.equal(paramErrorConfig.user_name_inappropriate.name,
      responseHelper.paramValidationError('test_1', api_error_key, [param_error_key], {}).toHash().err.param[0].name);
  });

  it('Should return message in param object as per code in input', function(){

    assert.equal(paramErrorConfig.user_name_inappropriate.message,
      responseHelper.paramValidationError('test_1', api_error_key, [param_error_key], {}).toHash().err.param[0].msg);
  });
